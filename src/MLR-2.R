...
ourPredict = function(df)
{
  ...
  #data_part <- createDataPartition(y = df$Attrition,  p = 0.70, list = F)
  
  employee_factor <- data.frame(df)
  ...
  test <- df[-train_rows,] # 30% data goes here
  train <- df[train_rows,] # 70% here
