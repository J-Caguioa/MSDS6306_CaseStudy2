...
# Add numeric columns for Likert scale-type variables.
copyAsNumeric <- function(df, cols) {
  newNumeric <- vector()
  for (col in cols) {
    colNum <- paste0(col, "Num")
    df[, (colNum)] <- as.numeric(col)
    newNumeric <- c(newNumeric, as.character(colNum))
  }
  return(newNumeric)
}

copyAsNumeric(employee_factor, c("EnvironSat", "JobSat", "RelationSat", "WorkLifeBal"))

# Create Life Satisfaction variable from 
employee_factor$LifeSat <- rowMeans(employee_factor[ , newNumeric])
                                                     #c("EnvironSatNum", "JobSatNum", "RelationSatNum", "WorkLifeBalNum")])

ggplot(employee_factor, aes(x=Age, y=LifeSat)) +
  geom_point(alpha=0.3) +
  xlab("Age (years)") +
