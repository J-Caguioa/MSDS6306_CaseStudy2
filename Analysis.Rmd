...
package_list <- c("tidyverse", "readxl", "knitr", "mlr", "kableExtra", "corrplot", "ggmosaic", "magrittr", "caret", "randomForest")
...
logvar <- function(df, cols) {
  for (col in cols) {
    name_log <- paste0(col, "Log")
    df[, (name_log)] <- log(df[, col])
  }
  return(df)
}

employee_numeric$HourRateLog <- log(employee_numeric$HourlyRate)
employee_numeric$DayRateLog <- log(employee_numeric$DailyRate)
employee_numeric$MonthRateLog <- log(employee_numeric$MnthRate)
employee_numeric$MonthIncLog <- log(employee_numeric$MnthInc)

#logvar(employee_numeric, c("HourlyRate", "DailyRate", "MnthRate", "MnthInc"))

ggplot(employee_factor, aes(x=HourlyRate, y=DailyRate)) +
  geom_point()

ggplot(employee_factor, aes(x=HourlyRate, y=MnthRate)) +
  geom_point()

ggplot(employee_factor, aes(x=HourlyRate, y=MnthInc)) +
  geom_point()

ggplot(employee_factor, aes(x=MnthRate, y=MnthInc)) +
  geom_point()

ggplot(employee_factor, aes(x=HourlyRate, y=MnthInc)) +
  geom_point()

pairs(~HourlyRate+DailyRate+MnthRate+MnthInc, data=employee_factor)

rate_lm <- lm(MnthInc ~ HourlyRate + DailyRate + MnthRate, employee_factor)
summary(rate_lm)

cor(employee_factor$HourlyRate, employee_factor$DailyRate)
cor(employee_factor$HourlyRate, employee_factor$MnthRate)
cor(employee_factor$HourlyRate, employee_factor$MnthInc)
cor(employee_factor$MnthRate, employee_factor$MnthInc)

pairs(~HourRateLog+DayRateLog+MonthRateLog+MonthIncLog, data=employee_numeric)

```

```{r rate-income2}

mean(employee_numeric$HourlyRate < employee_numeric$DailyRate)
mean(employee_numeric$HourlyRate < employee_numeric$MnthRate)
mean(employee_numeric$HourlyRate < employee_numeric$MnthInc)
mean(employee_numeric$MnthRate > employee_numeric$MnthInc)

summary(employee_factor[which(employee_factor$MnthRate < employee_factor$MnthInc), ])

```

For all observations, Monthly Rate is greater than Daily Rate, which is greater than Hourly Rate. However, only 82.4% of the observations have Monthly Rate greater than Monthly Income.
...
ggplot(employee_numeric, aes(x=Age, y=LifeSat)) +
  ...
  ggtitle("Life Satisfaction Tester") +
  theme(axis.ticks = element_blank(),
        plot.title = element_text(hjust = 0.5)) +
  geom_smooth(method = "lm",
              se = F,
