...
The decision tree fares slightly better than the k-NN model, with 85.29% overall accuracy and 32.20% sensitivity to Attrition. Still, this is not particularly good for identifying attriting employees. Among the used variables are Age, Environment Satisfaction, Job Role, Monthly Income, Overtime, Stock Option Level, and Total Working Years.

A random forest model is an ensemble of decision trees. Does the crowd's perforamnce exceed that of the individual?

```{r attrition-randomforest}

# Create random forest model on training data, test on testing data.
set.seed(123)
employee_rf <- randomForest(Attrition ~ ., data=train_subset, ntree=500, mtry=8, importance=T, method="class")
#summary(employee_rf)
# plot(employee_rf)
# print(employee_rf)
...
confusionMatrix(prediction_rf, test_subset$Attrition, positive="Yes")

```

Unfortunately, the random forest performs worse at detecting true positives, with only 27.12% sensitivity compared to the individual decision tree's 32.20%. This model lists Overtime, Monthly Income, Total Work Years, Job Role, and Age as its overall most important predictors. For predicting those who attrite specifically, Overtime, Stock Option Level, Monthly Income, Work-Life Balance, Age, Marital Status, and Job Level are most significant.
...
Our recommendations for next steps and other variables needed are as follows:
