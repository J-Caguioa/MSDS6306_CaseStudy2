...
job_att_percent_bar <- employee_factor %>%
  ...
  coord_flip() +
  scale_fill_manual(values=hardcolors) +
  geom_text(aes(x=JobRole, y=freq, group=freq, label=freq), size=2) +
...
t(ddply(employee_factor, .(Attrition), summarize,
  ...
  kable(caption="Attrition: Means for Numeric Variables", align="c", table.attr="style='width:50%;'") %>%
  kable_styling(bootstrap_options=c("striped", "hover", "condensed"), font_size=7)
...
NomBar <- function(df, xvar="group1", yvar="group2") {
  ggplot(df, aes_string(x=xvar, y=..count.., fill=yvar)) +
    geom_bar(position="dodge", stat="identity")
}

#NomBar(employee_factor, "Gender", "Attrition")
#ggplot(employee_factor, aes_string(x=Gender, y=..count..)) +
 #   geom_bar(position="dodge", stat="identity")

ggplot(employee_factor, aes(x=BusTravel, y=..count..)) +
