...
lm2 <- lm(LogIncome ~ Age, employee_log)
par(mfrow=c(2,2))
plot(lm2)
summary(lm2)
...
The correlation for Age and log(Monthly Income) drops slightly from before, with Pearson's r = `r cor(employee_log$Age, employee_log$LogIncome)`. However, the residual diagnostics look much better. 
...
```{r jobrole1, warning=F, message=F}
...
```{r attrition2, include=F, results="hide", fig.show="hide"}
...
grid.arrange(att_age, att_distance, att_monthlyincome, att_salaryincrease, 
             att_numcompanies, att_totalyears, att_monthlyincome, att_yearscomp, 
...
predict_rf <- predict(employee_rf, newdata=test_subset, type="class")
confusionMatrix(test_subset$Attrition, predict_rf, positive="Yes")

```

```{r echo=T, random_forest_predict}
employee_rf_predict <- predict(employee_rf, newdata=test_subset, type="class")
table(test_subset$Attrition, employee_rf_predict)
```

```{r kmeans}

# employee_kmeans <- kmeans(employee_numeric, centers=5, nstart=20)
# print(employee_kmeans)
...
```{r eda-attrited-plots1, message=F, warning=F, include=F, results="hide"}
...
```{r eda-attrited-plots2, message=F, warning=F, include=F, results="hide"}
# Exploratory bar graphs of factor variables.
attrited %>%
  keep(is.factor) %>%
  gather() %>%
...
```{r eda-attrited-lessthan2yearsAtComp, message=F, warning=F, include=F, results="hide"}
...
```{r eda-attrited-lessthan2yearsAtComp2, message=F, warning=F, include=F, results="hide"}
# Exploratory bar graphs of factor variables.
attrited.atCoLessThan2Yrs %>%
  keep(is.factor) %>%
  gather() %>%
...
```{r attrition-drilldown, include=F, results="hide"}
