...
package_list <- c("tidyverse", "readxl", "knitr", "mlr", "kableExtra", "corrplot", "ggmosaic", "magrittr", "caret", "randomForest", "reshape", "gridExtra", "GGally", "purrr", "cowplot", "rpart", "rpart.plot", "leaps")
...
# Reusable indicator colors for attrition.
...
A moderate positive correlation (Pearson's r = 0.498) exists between age and monthly income. Drawing a scatterplot between the two shows a general upward trend in income as age increases, and the best-fitting regression line has a positive slope. However, a linear relationship may not necessarily be an optimal model for these variables.
...
jobcolors <- c("#E6194B", "#3CB44B", "#FFE119", "#4363D8", "#F58231", "#911EB4", "#42D4F4", "#F032E6", "#BFEF45")

# Function that ggplots boxplots. Takes nominal groups on x-axis. Prefers continuous variable on y-axis.
ContVsNomBoxplots <- function(df, xvar="groups", yvar="continuous") {
  ggplot(df, aes_string(x=xvar, y=yvar, group=xvar, fill=xvar)) +
    geom_boxplot() +
    scale_fill_manual(values=jobcolors) +
...
# 
# 
# #grid.arrange(job_mnthinc, job_yearscomp, job_att, job_co_worked, job_salinc, ncol=2)
# 
# legend <- get_legend(job_mnthinc)
# plot_grid(job_mnthinc +theme(legend.position="none"), job_yearscomp, legend, align='vh', labels=c("A","B"), hjust=-1, nrow=1)
# 
# job_att
...
prow <- plot_grid(job_envsat+theme(legend.position="none"),
                  job_jobsat+theme(legend.position="none"),
                  job_relsat+theme(legend.position="none"),
                  align='vh', hjust=-1, nrow=1)
legend2 <- get_legend(job_envsat)
plot_grid(prow, legend, rel_widths=c(4, .3))
...
employee_factor %>%
#  mutate(BusTravel=fct_relevel(BusTravel,"Non-Travel", "Travel_Rarely", "Travel_Frequently")) %>%
  ggplot(aes(x=BusTravel, y=..count..)) +
...
ggplot(employee_factor, aes(x=reorder(JobRole, desc(JobRole)), y=..count..)) +
...
```{r attrition0, fig.width=1, fig.height=1}
...
att_hourlyrate <- ggplot(employee_factor, aes(x=HourlyRate, color=Attrition)) +
  geom_density(alpha=0.3) +
  xlab("Hourly Rate") +
  ylab("Density") +
  scale_color_manual(values=attcol)

# Attrition and daily rate.
att_dailyrate <- ggplot(employee_factor, aes(x=HourlyRate, color=Attrition)) +
  geom_density(alpha=0.3) +
  xlab("Hourly Rate") +
  ylab("Density") +
  scale_color_manual(values=attcol)

# Attrition and monthly rate.
att_monthlyrate <- ggplot(employee_factor, aes(x=HourlyRate, color=Attrition)) +
  geom_density(alpha=0.3) +
  xlab("Hourly Rate") +
  ylab("Density") +
  scale_color_manual(values=attcol)
...
att_monthlyincome <- ggplot(employee_factor, aes(x=MnthInc, color=Attrition)) +
  geom_density(alpha=0.3) +
  xlab("Monthly Income") +
  ylab("Density") +
  scale_color_manual(values=attcol)

# Attrition and percent salary increase.

# Attrition and stock option levels.

grid.arrange(att_hourlyrate, att_monthlyincome, ncol=2)
...
# Attrition and training sessions in the previous year.
...
# Attrition and years in current role.
...
att_jobbus <- mosaicplot(Attrition ~ BusTravel,
                         data=employee_factor,
                         color=T,
