...
kable(summary(descriptiveStats1), format='markdown', col.names=c("Age", "Monthly Income", "% Salary Increase", "Years at Company")) %>% kable_styling(bootstrap_options = c("striped", "hover"))

kable(summary(descriptiveStats2), format='markdown', col.names=c("Years at Current Role", "Years Since Promotion", "Years with Current Manager")) %>% kable_styling(bootstrap_options = c("striped", "hover"))
...
The followng is a histrogram showing the distribution of Age in the company.  Overlayed on top of the histogram is the density curve and can see that the ages are pretty normally distributed
```{r echo=T}
# Histogram - age
# 
summary(employee$Age)
...
The following is a histrogram of Monthly Income in the company with an overlay of the density curve showing the distribution of income.  The distribution appears to be right skewed with a higher amount of the monthly income 
```{r, echo=T}
# Histogram - Income
summary(employee$MnthInc)
